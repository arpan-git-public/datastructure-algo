...
public class LT33SearchInRotatedSortedArray {
    ...
    public int search(int[] nums, int target) {
        //identify the pivot index for sorted array
        ...
        //Then do binary search on segments.
        int foundIndex =  binarySearch(nums, low, nums.length -1,target) ;
         if(foundIndex != -1) return foundIndex;
         return binarySearch(nums, 0, low-1,target);
    }


    public int searchInBinary(int[] nums, int target) {
        int low = 0, high = nums.length - 1;
        while( low <= high) {
            int mid = low + (high - low) / 2;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(lt.searchInBinary(nums,target));
    }
}
